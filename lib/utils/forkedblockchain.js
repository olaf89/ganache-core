...
ForkedBlockchain.prototype.createVMFromStateTrie = function() {
  const vm = BlockchainDouble.prototype.createVMFromStateTrie.apply(this, arguments);
  this.patchVM(vm);
  return vm;
};
...
ForkedBlockchain.prototype.patchVM = function(vm) {
  ...
  vm.stateManager._lookupStorageTrie = this.getLookupStorageTrie();
...
const storageTrieCache2 = {};
ForkedBlockchain.prototype.getLookupStorageTrie = function() {
  // close over this object so the _lookupStorageTrie fn below can use this cache
  const storageTrieCache = {};

  return function _lookupStorageTrie(address, callback) {
    address = to.hex(address);

    let storageTrie = storageTrieCache[address];
    if (storageTrie) {
      storageTrieCache2[address] = storageTrie;
      return callback(null, storageTrie);
    }
    storageTrie = storageTrieCache2[address];
    if (storageTrie) {
      storageTrieCache[address] = storageTrie;
      return callback(null, storageTrie);
    }
    storageTrie = this.createForkedStorageTrie(address);
    storageTrieCache[address] = storageTrie;
    storageTrieCache2[address] = storageTrie;
...
ForkedBlockchain.prototype.getStorage = function(vm, address, key, number, callback) {
  vm.stateManager._lookupStorageTrie(address, function(err, trie) {
    if (err) {
      return callback(err);
    }
